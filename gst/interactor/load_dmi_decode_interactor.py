...
@singleton
class LoadDmiDecodeInteractor:
    @inject
    def __init__(self, dmi_decode_repository: DmiDecodeRepository) -> None:
        self._dmi_decode_repository = dmi_decode_repository

    def execute(self, system_info: SystemInfo) -> Observable:
        return reactivex.defer(lambda _: reactivex.just(self._dmi_decode_repository.refresh(system_info)))
