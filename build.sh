...
function build_flatpak {
	...
	desktop-file-validate build/flatpak/build/files/share/applications/com.leinardi.gst.desktop || exit $?
}
...
appstream-util validate-relax data/com.leinardi.gst.appdata.xml || exit $?
appstream-util appdata-to-news data/com.leinardi.gst.appdata.xml | sed '/^~*$/s/~/=/g' > CHANGELOG.md
...
if [[ ${FLATPAK_REMOTE} -eq 1 ]]; then
	...
	desktop-file-validate build/meson/data/com.leinardi.gst.desktop && \
